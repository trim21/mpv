...
static mpv_handle* jclient(JSContext* ctx)
{
    return jctx(ctx)->client;
}
...
{
    switch (node->format) {
    case MPV_FORMAT_NONE: return JS_NULL;
    case MPV_FORMAT_STRING: return JS_NewString(ctx, node->u.string);
    case MPV_FORMAT_INT64: return JS_NewInt64(ctx, node->u.int64);
    case MPV_FORMAT_DOUBLE: return JS_NewFloat64(ctx, node->u.double_);
    case MPV_FORMAT_FLAG: return JS_NewBool(ctx, node->u.flag);
    ...
    default: return JS_ThrowTypeError(ctx, "Unsupported mpv node format");
    }
}
