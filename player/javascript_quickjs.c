...
static JSValue js_write_file(JSContext *ctx, JSValueConst this_val, int argc,
  ...
  const char *fname = js_to_cstring(ctx, argv[1]);
  const char *data = js_to_cstring(ctx, argv[2]);
  ...
  fname += strlen(prefix);
  fname = mp_get_user_path(NULL, jctx(ctx)->mpctx->global, fname);
...
static void add_package_fns(JSContext *ctx, JSValue obj, const char *module,
                            const struct fn_entry *e) {
  JSValue mod = JS_NewObject(ctx);
  for (int n = 0; e[n].name; n++) {
    JS_SetPropertyStr(ctx, mod, e[n].name,
                      JS_NewCFunction(ctx, e[n].fn, e[n].name, e[n].length));
  }
  JS_SetPropertyStr(ctx, obj, module, mod);
}

static int setup_functions(JSContext *ctx, struct script_ctx *sctx) {
  JSValue global = JS_GetGlobalObject(ctx);
  JSValue mp = JS_NewObject(ctx);
  add_package_fns(ctx, mp, "mp", main_fns);
  JSValue mp_obj = JS_GetPropertyStr(ctx, mp, "mp");
  add_package_fns(ctx, mp_obj, "utils", utils_fns);

  JS_SetPropertyStr(ctx, mp_obj, "script_name",
                    JS_NewString(ctx, mpv_client_name(sctx->client)));
  JS_SetPropertyStr(ctx, mp_obj, "script_file",
                    JS_NewString(ctx, sctx->filename));
  if (sctx->path)
    JS_SetPropertyStr(ctx, mp_obj, "script_path",
                      JS_NewString(ctx, sctx->path));

  JS_SetPropertyStr(ctx, mp, "mp", mp_obj);
  JS_SetPropertyStr(ctx, global, "mp", mp_obj);

  JS_FreeValue(ctx, mp_obj);
